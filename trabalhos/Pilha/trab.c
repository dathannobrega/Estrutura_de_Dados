...
//funcao booleana que verifica se a pilha está vazia
bool empty(Stack *S){
    return S->top == -1;
}

//funcao booleana que verifica se a pilha esta cheia
bool full(Stack *S){
    return S->top == MAX-1;
}
    ...
    for (int i = ini-1, j=1; j <= vertice; j++){
        ...
        */
        if(matriz[i][j] == 0) continue;
            ...
            i = j;
            j = -1;
            //o j volta sendo -1 pois quando voltar o loop ele começa no zero. (j++)
        }
    }
}
...
>>>>>>> ef5bbc49cf7188980ab82d3962b817faefe8d91e
}