...
ArvAVL* cria_ArvAVL(){ //Cria ae aloca a arvore apontados os 2 ponteiros para nulo
...
void libera_NO(struct NO* no){ // apaga toda a arvore AVL 
    if(no == NULL)
        return;
    libera_NO(no->esq);
    libera_NO(no->dir);
...
int fatBalRec(ArvAVL p) {
if(p->dir == NULL && p->esq == NULL) return 0;
else {
  if(p->dir == NULL && p->esq != NULL)
       return fatBalRec(p->esq)+1;
  else
     if(p->dir !=NULL && p->esq == NULL)
        return fatBalRec(p->dir)+1;
     else
        return labs(fatBalRec(p->esq)-fatBalRec(p->dir));
  }
}
...
void emOrdem_ArvAVL(ArvAVL *raiz){
    ...
    if(*raiz != NULL){
        ...
        printf("No %d: H(%d) fb(%d) fbn(%d)\n",(*raiz)->info,altura_NO(*raiz),fatorBalanceamento_NO(*raiz),fatBalRec(*raiz));
...
struct NO* procuraMenor(struct NO* raiz){
    ...
    if (estaVazia_ArvAVL(&raiz)) return NULL;
    atual = raiz;
    prox = raiz->esq;
    while(prox != NULL){
        atual = prox;
